...
import { Plus, Minus, ShoppingCart, X, Scan } from 'lucide-react';
...
interface Props {
  ...
  isLoading: boolean;
  onClose: () => void;
  onAdd: (items: TempItem[]) => void;
}

export const AddProductsToLayawayPOS: React.FC<Props> = ({ products, isLoading, onClose, onAdd }) => {
  ...
  const total = tempItems.reduce((sum, item) => sum + item.salePrice * item.quantity, 0);
